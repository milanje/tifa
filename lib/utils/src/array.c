...
mpz_array_t* alloc_mpz_array(uint32_t length) {
    ...
    //
    // _WARNING_: The mpz_t array given by array->data is fully mpz_init'ed 
    //            once and for all to avoid possible memory deallocations
    //            and reallocations. 
    //
    for (uint32_t i = 0U; i < array->alloced; i++) {
...
void free_mpz_array(mpz_array_t* array) {
    if (array->alloced != 0U) {
        //
        // _WARNING_: The mpz_t array given by array->data is fully 
        //            mpz_init'ed once and for all. Consequently
        //            array->data has to be completely mpz_clear'ed (and not 
        //            just up to array->length as in older versions).
        //
        uint32_t alloced = array->alloced;
        mpz_t*   data    = array->data;
        
        for (uint32_t i = 0U; i < alloced; i++) {
            mpz_clear(data[i]);
        }
        free(array->data);
        free(array);
